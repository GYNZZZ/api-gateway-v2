...
const elements = {};
for (const id of ["adminKey","toggleKey","loadButton","refreshButton","logoutButton","connectionBadge","notice","userCount","totalBalance","logCount","successCount","failureCount","todayCount","totalCharged","totalTokens","promptTokens","completionTokens","usersBody","logsBody","usersMeta","logsMeta","createForm","createButton","newName","newBalance","createdKey","createdKeyValue","dismissCreatedKey","topupDialog","topupForm","topupUserName","topupAmount","confirmTopup","closeTopup","cancelTopup","maintenanceToggle","defaultModelSelect","rateLimitToggle","perUserPerMinute","globalPerMinute","saveSettingsButton","providersBody","providerForm","providerId","providerName","providerBaseUrl","providerApiKeyEnv","modelsBody","modelForm","modelId","modelName","modelProviderId","modelPriceMultiplier","modelIsDefault"]) elements[id] = document.querySelector(`#${id}`);
...
function renderMetrics() {
  ...
  for (const [id, value] of Object.entries(values)) elements[id].textContent = formatNumber(value);
  elements.logoutButton.hidden = false;
}

function renderUsers() {
  elements.usersMeta.textContent = `${users.length} 个用户`;
  elements.usersBody.innerHTML = users.length ? users.map((user) => `<tr><td>#${escapeHtml(user.id)}</td><td><span class="user-name">${escapeHtml(user.name)}</span></td><td><code class="key-chip">${escapeHtml(user.keyPreview || "-")}</code><span class="key-status ${user.apiKeyEnabled ? "enabled" : "disabled"}">${user.apiKeyEnabled ? "已启用" : "已禁用"}</span></td><td><span class="balance">${formatNumber(user.balance)}</span></td><td class="align-right"><span class="user-actions"><button class="topup-button" data-action="topup" data-user-id="${user.id}">充值</button><button class="topup-button" data-action="toggle" data-user-id="${user.id}">${user.apiKeyEnabled ? "禁用" : "启用"}</button><button class="topup-button danger" data-action="rotate" data-user-id="${user.id}">轮换 Key</button></span></td></tr>`).join("") : '<tr><td colspan="5"><div class="empty-state">暂无用户</div></td></tr>';
}

function renderLogs() {
  elements.logsMeta.textContent = `${logs.length} 条`;
  elements.logsBody.innerHTML = logs.length ? logs.map((log) => `<tr><td>${escapeHtml(new Date(log.timestamp).toLocaleString("zh-CN"))}</td><td>${escapeHtml(log.userName || log.userId || "-")}</td><td>${escapeHtml(`${log.method || ""} ${log.route || ""}`.trim())}</td><td>${escapeHtml(log.model || "-")}</td><td><span class="status-pill ${isSuccess(log) ? "success" : "error"}">${escapeHtml(log.status)}</span></td><td>${formatNumber(log.charged)}</td><td>${formatNumber(tokenValue(log, "total_tokens"))}</td><td>${formatDuration(log.durationMs)}</td></tr>`).join("") : '<tr><td colspan="8"><div class="empty-state">暂无调用日志</div></td></tr>';
}

function modelStats(modelId) {
  const matching = logs.filter((log) => log.model === modelId);
  return { calls: matching.length, tokens: matching.reduce((sum, log) => sum + tokenValue(log, "total_tokens"), 0) };
}
...
function renderConfiguration() {
  ...
  elements.modelsBody.innerHTML = models.length ? models.map((model) => { const stats = modelStats(model.id); const price = Number.isFinite(Number(model.priceMultiplier)) ? Number(model.priceMultiplier) : 1; const pricing = model.pricing || {}; const input = Number(pricing.baseInput) || 0; const output = Number(pricing.baseOutput) || 0; const multiplier = Number(pricing.saleMultiplier) > 0 ? Number(pricing.saleMultiplier) : 1; return `<tr><td><code>${escapeHtml(model.id)}</code></td><td>${escapeHtml(model.name)}</td><td>${escapeHtml(model.providerId)}</td><td><span class="key-status ${model.enabled ? "enabled" : "disabled"}">${model.enabled ? "是" : "否"}</span></td><td>${model.isDefault ? '<span class="key-status enabled">是</span>' : "否"}</td><td>${formatNumber(price)} 点 / 次</td><td>${formatUsd(input)}</td><td>${formatUsd(output)}</td><td>${formatNumber(multiplier)} 倍</td><td>${formatUsd(input * multiplier)}</td><td>${formatUsd(output * multiplier)}</td><td>${formatNumber(stats.calls)}</td><td>${formatNumber(stats.tokens)}</td><td class="align-right"><span class="user-actions"><button class="topup-button" data-model-action="pricing" data-model-id="${escapeHtml(model.id)}">调整倍率</button><button class="topup-button" data-model-action="toggle" data-model-id="${escapeHtml(model.id)}">${model.enabled ? "禁用" : "启用"}</button><button class="topup-button" data-model-action="default" data-model-id="${escapeHtml(model.id)}">设为默认</button></span></td></tr>`; }).join("") : '<tr><td colspan="14"><div class="empty-state">暂无模型</div></td></tr>';
}
...
async function createModel(event){event.preventDefault();try{await api("/admin/models",{method:"POST",body:JSON.stringify({id:elements.modelId.value.trim(),name:elements.modelName.value.trim(),providerId:elements.modelProviderId.value,enabled:true,isDefault:elements.modelIsDefault.checked,priceMultiplier:Number(elements.modelPriceMultiplier.value)})});elements.modelForm.reset();elements.modelPriceMultiplier.value="1";await loadConfiguration();showNotice("模型已创建。");}catch(e){showNotice(e.message,true,true);}}
...
async function submitTopup(event){event.preventDefault();if(!selectedUser)return;const amount=Number(elements.topupAmount.value);if(!Number.isFinite(amount)||amount<=0)return showNotice("充值点数必须大于 0。",true);setButtonLoading(elements.confirmTopup,true);try{const user=await api(`/admin/users/${selectedUser.id}/topup`,{method:"POST",body:JSON.stringify({amount})});closeTopup();showNotice(`${user.name} 的余额已更新为 ${formatNumber(user.balance)}。`);await loadUsers();}catch(e){showNotice(e.message,true,true);}finally{setButtonLoading(elements.confirmTopup,false);}}
...
function logout(){
  ...
  elements.modelsBody.innerHTML='<tr><td colspan="14"><div class="empty-state">连接后加载模型</div></td></tr>';
